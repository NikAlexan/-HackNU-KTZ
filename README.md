# Виртуальный цифровой двойник локомотива

Проект разработан для кейса **КТЖ (Казахстан Темір Жолы)** по теме «Виртуальный цифровой двойник локомотива».

Система реализует полный стек цифрового двойника подвижного состава: бортовой модуль сбора и обработки телеметрии, транспортный слой на базе MQTT, серверную платформу агрегации данных и диспетчерский веб-дашборд. В основе — физическая модель двух типов локомотивов парка КТЖ: электровоза **KZ8A** (Alstom Prima T8) и тепловоза **TE33A** (GE Evolution Series).

Цифровой двойник решает задачи:
- непрерывного мониторинга состояния агрегатов в реальном времени;
- оценки накопленного износа по данным с датчиков с использованием пороговых значений из технической документации;
- раннего обнаружения отклонений (перегрев, просадка напряжения КС, отказ турбонагнетателя и др.);
- поддержки решений об обслуживании — когда и какой узел требует ремонта.
